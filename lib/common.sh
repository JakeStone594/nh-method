...
# A netdev name says nothing about which radio it is.
driver_of()    { basename "$(readlink -f "/sys/class/net/${1:-}/phy80211/device/driver" 2>/dev/null)" 2>/dev/null; }
# An AP vif has no carrier until hostapd beacons, so "did my link-up take" is
# the admin flag (IFF_UP), never operstate.
iface_admin_up() { local f; f=$(cat "/sys/class/net/${1:-}/flags" 2>/dev/null) || return 1
                   [ -n "$f" ] && [ $(( f & 1 )) -eq 1 ]; }
...
ip6tables_legacy() { ip6tables -V 2>&1 | matches -F '(legacy)'; }

# Reports, never refuses: no tool here writes a v6 rule, so a wrong v6 backend cannot
# affect the v4 run in front of you. What it is for is that the v6 pin is a HAND pin --
# nothing re-asserts it the way the app re-pins v4 at boot -- so it can flip between
# runs, and a v6 rule written by hand afterwards lands where netd never reads it.
v6_backend() {
    if ip6tables_legacy; then ok "ip6tables legacy backend"
    else
        warn "ip6tables is not the legacy backend — a v6 rule would land in a ruleset netd never reads"
        cmd "update-alternatives --set ip6tables /usr/sbin/ip6tables-legacy"
        return 1
    fi
    [ "$(cat /proc/sys/net/ipv6/conf/all/forwarding 2>/dev/null)" = 0 ] && return 0
    warn "ipv6 forwarding is on and nothing here installs a v6 rule — v6 traffic is unfiltered"
    return 1
}

# rc >=2 is a probe that did not run; reporting it as "absent" is how a teardown
# reports a chain removed it never looked at. A wrong backend answers rc 1.
chain_exists() {
    local rc
    iptables -t "${1:-filter}" -S "${2:-}" >/dev/null 2>&1; rc=$?
    [ "$rc" -ge 2 ] && { warn "probe did not run (iptables rc $rc): iptables -t ${1:-filter} -S ${2:-}"; return 2; }
    [ "$rc" -eq 0 ]
}

# ---------------------------------------------------------------- validation

# A captive-portal POST is a login only if it carries a field name of this shape:
# the portal logs every POST, so telemetry and binary TLS blobs arrive as field
# names too. Exported because rogueap and rogueap-menu must filter identically.
export NH_FORM_FIELD='^[A-Za-z0-9_.\[\]-]{1,40}$'
...
# The portal substitutes url_for() itself; any other jinja tag left in a template
# renders literally. Shared so the menu offers only what rogueap will accept.
template_renderable() {
    ! sed -E 's/\{\{[^}]*url_for\([^)]*\)[^}]*\}\}//g' "${1:-}" 2>/dev/null |
      /usr/bin/grep -qE '\{\{|\{%'
}
