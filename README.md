...
⚠ **These are installed tools, not clone-and-run scripts.** They source the shared library from
the path it occupies once installed — `/usr/local/lib/nh/common.sh` — and not from `lib/` in this
repository, so `bin/androute` run straight from a clone exits on its fourth line naming a path that
does not exist. That is the mapping working as intended rather than a defect: this repo mirrors
files that live elsewhere on a device, and `contract/REGRESSION.md` Q83 documents the mapping
precisely so the opposite mistake is caught — repointing a tool at the clone so it "works" in a
place it was never meant to run, which drifts the mirror while `git status` stays clean. To run
one, install `lib/common.sh` to that path first.
