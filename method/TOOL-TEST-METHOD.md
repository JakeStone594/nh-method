...
**Related:** `method/CODING-RULES.md` (the authoring contract — a campaign tests conformance to
...
⛔ **AND THE ASK MUST SAY WHAT THE TEST SETTLES.** Naming the kit is not the request; it is the
shipping label. State **(a)** the question that cannot be answered without it, **(b)** what a yes and
a no each unlock, and **(c)** whether anything is blocked meanwhile. ⚠ **An operator who cannot see
(a) is being asked to fetch hardware on trust, and is right to refuse** — *"I know it works if I
connect it, whats the point?"* is the correct response to a request that names only the object.

⛔ **IF THE (a) SENTENCE COMES OUT EMPTY, THAT IS THE FINDING — DO NOT MANUFACTURE ONE.** Either the
test is genuinely redundant and should be dropped, or you have mis-classified it and that sentence is
what reveals it. ⭐ **The write-up of what a test settles is the check on whether it settles
anything**, so it is owed to you before it is owed to the operator.

⚠ **Distrust your own *"this is only confirmatory"* hardest.** A test can be confirmatory of the
mechanism and still cross a boundary the proven part never touched — most sharply, from *does this
work?* to *does it work while another OS owns the same device?* `KERNEL-V2.md` §HIDRAW is the worked
instance: the half dismissed as unsurprising returned the only non-obvious result, and it was the
**opposite** of the neighbouring subsystem.

⛔ **AND STATE WHAT THE TEST MIGHT COST THE ITEM ITSELF — A BORROWED PERIPHERAL CAN COME BACK
CHANGED.** Added 2026-08-21, after a test **permanently unpaired the operator's daily-use mouse from
its wireless receiver**: a multi-host device holds ONE pairing, the test's own pairing displaced the
existing one, and undoing it needed software on another machine. ⚠ **The damage was not to the thing
under test and appeared only AFTER the campaign had been scored a success**, so nothing in the run
reported it. ⛔ **The failure is in the ASK, not in the procedure: "may I borrow your mouse" and "may
I re-pair your mouse" are different requests, and only the second one was true.** ⭐ **So the kit ask
carries a fourth clause beside (a) the question, (b) what a yes/no unlocks and (c) what is blocked:
(d) WHAT STATE THE ITEM MAY NOT COME BACK IN** — and prefer an item the operator can afford to
reconfigure. ⚠ **Distrust "it is just plugging it in" exactly as hard as "it is only confirmatory"**:
pairing, firmware handshakes, channel selection and profile switches all persist in the peripheral
after the test ends. Worked instance: `BLUETOOTH.md` §HID host role.
...
⛔ **SNAPSHOT `/proc/sys/kernel/random/boot_id` — ONE LINE, AND IT IS THE ONLY THING THAT MAKES A
REBOOT VISIBLE TO THE CAMPAIGN.** §8 has required a `boot_id` on carry-forward since 2026-08-10;
**this section did not, which is the half a campaign author reads while writing a snapshot block.**
⚠ **A reboot and a clean teardown are INDISTINGUISHABLE from the after-state** — `con_mode` back to
0, no chains, no vif, no daemons — and `/run` here is **persistent f2fs**, so even a
`state.torn-down` survives and reads as evidence the teardown worked. **A campaign that does not
record its boot_id cannot tell its own discipline from a reboot.**
⚠ **And the routing ids move across one:** a fresh boot re-instantiates the network and re-allocates
Android's policy table number, which is expected and is **not** a `P-60` instability sample.
Instance: `/root/tests/runA-first-dhcpack-2026-08-25/RESULTS.md` §*A reboot intervened*, 2026-08-26 —
the boundary was found by the curator at harvest, not by the campaign that straddled it.
...
2. **`rc=$?` after a pipeline is the status of the LAST element.** Capture the status of the command
   you are judging: **`$pipestatus[1]` under zsh** (1-indexed), `PIPESTATUS` only inside an explicit
   `bash -c`, or judge by a functional line of the output. ⛔ **NOT a bare `PIPESTATUS` — this clause
   read *"or use `PIPESTATUS`"* until 2026-08-23 and that remedy is EMPTY here.** It is a **bash**
   array; the interactive shell and every assistant tool call are **zsh**, where `${PIPESTATUS[0]}`
   expands to **nothing**, with no error, and `rc=` reads as neither a pass nor a visible failure.
   Verified on-device 2026-08-23 with both controls: `bash -c` → `rc=0`; on a first element exiting
   7, zsh `$pipestatus[1]` → **7** while `${PIPESTATUS[0]}` stayed empty. `PRECONDITIONS.md`
   **P-96**. This box's shell rules make it worse — see the zsh word-splitting trap in T0's
   Conventions.
...
instrument. The first two instances below came from the `usbgadget` from-scratch rebuild, whose regression
...
| **the snapshot block itself is an instrument, and this campaign's manufactured THREE `[FAIL]`s** | added 2026-08-26 from `runA`. Both causes were **already owned by `PRECONDITIONS.md` and were re-derived at cost because they are filed under the PROBE, not under SNAPSHOT-WRITING**: `P-16` (`iptables-save > f 2>&1` splices stderr mid-rule — and its 2026-08-26 addition: the splice lands at a different offset in the before and after captures, so it reads as a **chain-name difference** and **survives the `P-49` normalisation**) and `P-49` (timestamps and policy counters — and its 2026-08-26 addition: `cmd wifi status` embeds RSSI and tx/rx totals, so the class is wider than iptables). ⛔ **The campaign's own stop condition was *any `[FAIL]` → do not continue*, so a clean teardown was one step from being recorded as a broken one.** ⭐ **Before putting a command in a snapshot block, ask what in its output moves on its own** — and assert the specific thing the test can change, not a whole dump. |
